...
- 動的なウェブサイトと違い、静的なウェブサイトの場合は、アクセス前に表示するページが決まっているため、上記流れの3に当たるユーザーに合わせた編集の流れがありません。
そのため、流れは１→２→４の順番になります。
...
- また、ApacheとNegixの違いは、<br>
Apacheはマルチプロセスのプロセス駆動アーキテクチャであり、各リクエストをプロセスに割り当てて処理を行うため、リクエストが大量に来た際に、プロセスが同時に起動するので負荷処理が非常に大きくなるというデメリットがあり、<br>
一方、Negixはシングルスレッドモデルのイベント駆動アーキテクチャであり、シングルスレッドでループ処理をまわし、キューに溜まったイベントを処理していく処理方式であるイベントループ方式で、少量のプロセスだけで大量のリクエストを処理することが可能という点に違いがあります。
